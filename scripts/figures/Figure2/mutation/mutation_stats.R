...
fail <- c("TGL49_0025_Cf_U_PE_334_TS", "TGL49_0209_Cf_U_PE_378_TS", "TGL49_0010_Cf_U_PE_334_TS")
...
#general 
descriptors <- c("total samples", "total patients", "postive samples", "negative samples", "patients with serial samples", "germline mutation", "germline CNV", "no germline", 
                 "pediatric samples", "pediatric patients", "adult samples", "adult patients")
stats <- c(cohort, patients, serial_patients, pos_samples, neg_samples, germline, germline_cnv, no_germline, pediatric_samples, pediatric, adult_samples, adult)
stats_cohort <- as.data.frame(cbind(descriptors, stats))
stats_cohort$percent <- as.numeric(stats_cohort$stats)/cohort*100
...
stats_negative$percent <- as.numeric(stats_negative$stats)/neg_samples*100

#positive
descriptors <- c("positive", "patients", "high grade samples", "low grade samples", "TP53 mutations", "samples with TP53 mutations", "patients with TP53 mutations", 
                 "high grade mutation positive", "low grade mutation positive", "patients with multiple TP53", "BRCA1", "BRCA2", "PALB2", "MLH1", "MSH2", "MSH6", "PMS2", "EPCAM", "APC")
stats <- c(pos_samples, pos_patients, high, low, pos_TP53_total, pos_TP53_samples, pos_TP53_patients, pos_high, pos_low, pos_TP53, 
...
stats_positive$percent <- as.numeric(stats_positive$stats)/pos_samples*100
