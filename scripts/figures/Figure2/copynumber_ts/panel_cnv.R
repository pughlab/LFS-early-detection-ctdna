...
### Normalize cnmops to non deleted samples
samples_norm <- samples[samples$cancer_status == "Negative" &
                          samples$previous_cancer == "no" &
                          samples$mutation_type == "Mutation", ]
cnmops_norm <- cnmops[, colnames(cnmops) %in% samples_norm$TS]
cnmops_norm <- as.matrix(cnmops_norm)
median_cn <- rowMeans2(cnmops_norm)
sd_cn <- rowSds(cnmops_norm)

cnmops_norm <- (cnmops_norm - median_cn)/sd_cn
cnmops_norm <- t(cnmops_norm)
cnmops <- (cnmops - median_cn)/sd_cn
cnmops <- t(cnmops)

### Normalize cnmops buffy coats
median_cn_bc <- rowMeans2(cnmops_bc)
sd_cn_bc <- rowSds(cnmops_bc)
cnmops_bc <- (cnmops_bc - median_cn_bc)/sd_cn_bc
cnmops_bc <- t(cnmops_bc)

### Normalize viscap to non deleted samples
viscap_norm <- viscap[ , colnames(viscap) %in% samples_norm$TS]
viscap_norm <- as.matrix(viscap_norm)
median_vis <- rowMeans2(viscap_norm)
sd_vis <- rowSds(viscap_norm)

viscap_norm <- (viscap_norm - median_vis)/sd_vis
viscap_norm <- t(viscap_norm)
viscap <- (viscap - median_vis)/sd_vis
viscap <- t(viscap)

### Normalize viscap buffy coats
median_vis_bc <- rowMeans2(viscap_bc)
sd_vis_bc <- rowSds(viscap_bc)
viscap_bc <- (viscap_bc - median_vis_bc)/sd_vis_bc
viscap_bc <- t(viscap_bc)

### Score cnmops for somatic deletions
cnmops_norm <- rowSums2(cnmops_norm)
...
### Score viscap for somatic deletions
viscap_norm <- rowSums2(viscap_norm)
...
### Merge cnmops data
cnmops <- rbind(cnmops, cnmops_bc)
viscap <- rbind(viscap, viscap_bc)
...
samples <- samples[order(factor(samples$mutation_type, levels = c("Deletion", "Duplication", "Mutation", "Lymphocytes")),
                         samples$ext_ID,
                         samples$timepoint), ]
cnmops <- cnmops[samples$TS, ]
viscap <- viscap[samples$TS, ]
...
col_cancer <- c(Positive = "#fb9a99", Negative = "#a6cee3", Lymphocytes = "#B2DF8A")
col_mutation <- c(Duplication = "#fb9a99", Deletion = "#a6cee3", Mutation = "grey65", Lymphocytes = "#B2DF8A")
...
annotation_legend = packLegend(list = list(Legend(title = "Germline Variant", 
                                                  at = c("Deletion", "Duplication", "Small Sequence Mutation", "Lymphocytes"),
                                                  legend_gp = gpar(fill = c("#a6cee3", "#fb9a99", "grey65", "#B2DF8A")),
                                                  #nrow = 1,
                                                  title_gp = gpar(fontsize = 8),
                                                  labels_gp = gpar(fontsize = 7),
                                                  grid_height = unit(1, "mm")),
                                           Legend(title = "Cancer Status", 
                                                  at = c("Negative", "Positive", "Lymphocytes"),
                                                  legend_gp = gpar(fill = c("#a6cee3", "#fb9a99", "#B2DF8A")),
                                                  #nrow = 1,
                                                  title_gp = gpar(fontsize = 8),
                                                  labels_gp = gpar(fontsize = 7),
                                                  grid_height = unit(1, "mm")),
...
pdf(file.path(outdir, "combined.pdf"), width = 5, height = 7)
...
cnmops_norm <- cnmops[row.names(cnmops) %in% samples_norm$TS, ]
...
viscap_norm <- viscap[row.names(viscap) %in% samples_norm$TS, ]
