...
samples <- samples[samples$ext_ID %in% samples$ext_ID[samples$cancer_status == "negative"], ]
...
samples$cancer_type <- factor(samples$cancer_type, levels = c("adrenal", "appendiceal_adenocarcinoma", "astrocytoma", "bladder", "breast", "endometrial", 
...
bottom_annotation <- HeatmapAnnotation("Cancer Type" = data_type,
                                       border = FALSE,
...
order <- c("LFS3", "LFS5", "LFS40", "LFS15", "LFS34", "LFS51", "LFS59", "LFS78", "LFS2", "LFS35", "LFS37", "LFS57", "LFS77", "LFS81", "LFS88", "LFS89")
col_split <- factor(col_split, levels = order)

## Generate heatmap
pdf(file.path(outdir, "integration_longitudinal_phenos.pdf"), width = 8.5, height = 3)
