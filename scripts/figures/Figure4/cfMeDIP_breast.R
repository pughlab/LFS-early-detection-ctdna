...
### Find files
data_matrix <- read_tsv(file.path(path, "signature_matrix_breast.tsv"))
data_scores <- read_tsv(file.path(path, "methylation_score_breast.tsv"))
...
top_annotation <- HeatmapAnnotation("Breast-Cancer\nScore" = anno_lines(data_anno,
                                                                     add_points = TRUE,
                                                                     pch = c(16, NA),
...
annotation_legend = packLegend(list = list(Legend(title = "Patient Type", 
                                           ...
                                           Legend(title = "Cancer Type",
                                                  at = c(c("Adenocarcinoma", "Adrenal", "Bladder", "Breast", "Endometrial",
                                                           "Glioma", "Lung", "Lymphoma", "Osteosarcoma", "Prostate", "Sarcoma")),
                                                  legend_gp = gpar(fill = c(c("#fd7f6f","#7eb0d5","#b2e061","#bd7ebe","#ffb55a", 
                                                                              "#beb9db","#fdcce5","#8bd3c7","#ebdc78","#b3d4ff","grey65"))),
                                                  nrow = 3,
                                                  title_gp = gpar(fontsize = 8),
                                                  labels_gp = gpar(fontsize = 7),
                                                  grid_height = unit(1, "mm")),
                                           ...
                                           Legend(title = "Mutations",
                                                  ...
                                                  labels_gp = gpar(fontsize = 7),
                                                  grid_height = unit(1, "mm")), 
                                           Legend(title = "Copy Number/Fragment",
...
pdf(file.path(path, "cfMeDIP_breast.pdf"), width = 10, height = 4)
